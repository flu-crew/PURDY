...
### The Human Data Set
**This data set is the best choice for someone who wants a fast test despite an extra processing step.**
The smallest test data set is the Human host H3N2 Influenza A data set from Wang et al. 2015 (https://www.frontiersin.org/articles/10.3389/fmicb.2015.00766/full). Fast5 data from which can be found at the European Nucleotide Archive under the accession PRJEB9812 (https://www.ebi.ac.uk/ena/browser/view/PRJEB9812). Soon PURDY will be updated to work with fast5 data, but for now you can retreive the fastq format files from NCBI under BioProject PRJEB9812. Seek out this bioproject on NCBI, then click on the linked SRA experiments to find the fastq files if this link does not work (https://www.ncbi.nlm.nih.gov/sra?linkname=bioproject_sra_all&from_uid=292765). You want the one with the accession labeled "ERX1051958" which represents all "2D reads" and not "ERX1053781". For how to retrieve sequence files from NCBI follow this guide: https://www.ncbi.nlm.nih.gov/sra/docs/sradownload. Unfortunately this data comes with a formatting error. It can be fixed using the Python script "formatHumanTestData.py" in the Scripts folder of this repository as follows:
```
python formatHumanTestData.py ERR974981_1.fastq ERR974981_1_reformat.fastq 
```

Here it's harder to tell what is true data because of low coverage but there is available data for barcodes 2,6,12,13,26,37,46,52,65,71,72,80,82,86, and 89. 

### The Swine Data Set
**This data set is the best choice for someone who wants a test with swine data of moderate size that can more accurately demonstrate the value of the classification function and does not require an extra processing step.**
...
### The Avian Data Set
**This data set is the best choice for someone who wants a large test data set with very deep coverage in an avian host.**
The largest test data set is the Avian host H5N1 Influenza A data set from Ip et al. 2023 (https://www.mdpi.com/2076-2607/11/2/529). This data can be downloaded from ScienceBase (https://doi.org/10.5066/P93VXVGO). We used all three highly pathogenic H5N1 avian MinION sequences using the library assembly technique referred to as “Method A” in Ip et al. 2023 (https://www.mdpi.com/2076-2607/11/2/529). Here the true data is in barcodes and 4-6. 

## 2) Install Dependencies and Run PURDY
This section will guide you through how to acquire PURDY, install PURDY's dependencies, and do a basic run with either a test data set or your data set. PURDY is written in bash and does not need to be installed or complied. See the next section to explore available arguments.
